// RUN: MLIRJNI_LIB_PATH=%MLIR_JNI_LIB_DIR%/libFIRRTLJNI.jni circel-java-test-launcher --class
// FIRRTLJNITests\$Test 2>&1 | FileCheck %s
    ...
    public static void main(String[] args) {
      ...
      try (Context context = Context.create()) {
        ...
        try (Builder builder = Builder.create(context)) {
          ...
          */
          FIRRTL.Operations.Module module =
              FIRRTL.Operations.Module.build(builder, loc, "test", scalarized);
          ...
          BlockArgument resetPort =
              module.addPort(builder, "reset", bitType, FIRRTL.Operations.Module.PortDirection.In);
          ...
          BlockArgument aOutPort = module.addPort(builder, "a_out", uintValueType,
              FIRRTL.Operations.Module.PortDirection.In);
          ...
          */
          ...
          Register x =
              Register.build(builder, loc, uintValueType, clockPort, "x", interestingName, false);
          Register y =
              Register.build(builder, loc, uintValueType, clockPort, "y", interestingName, false);
          ...
          */
          ...
          FIRRTL.PassManager passManager = FIRRTL.PassManager.create(builder);
          passManager.enableVerifier();
          passManager.addPrintIRPass();

          /*-
          // CHECK-NEXT: // Generated by CIRCT
          // CHECK-NEXT: module test(
          // CHECK-NEXT:   input        clock,
          // CHECK-NEXT:                reset,
          // CHECK-NEXT:   input [31:0] a,
          // CHECK-NEXT:                b,
          // CHECK-NEXT:   input        loadValues,
          // CHECK-NEXT:   input [31:0] a_out
          // CHECK-NEXT: );
          // CHECK:      endmodule
          */
          FirtoolOptions firtoolOptions = new FirtoolOptions();
          passManager.addFirtoolPasses(firtoolOptions);
