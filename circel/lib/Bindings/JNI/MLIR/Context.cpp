...
public:
  ...
  bool isValidForBuilderClass(JNIEnv *env, jclass jBuilderClass) {
    return env->IsSameObject(jBuilderClass, builderClass);
  }
  bool isValidForPassManagerClass(JNIEnv *env, jclass jPassManagerClass) {
    /**
     PassManager might be a subclass, so we use `IsAssignableFrom` instread of
     `IsSameObject`.
     */
    return env->IsAssignableFrom(jPassManagerClass, passManagerClass);
  }
  ...
  JNICache(JNIEnv *env)
      : contextConstructor(env, "MLIR/Context"),
        builderClass(globalReferenceToClass(env, "MLIR/Builder")),
        passManagerClass(globalReferenceToClass(env, "MLIR/PassManager")),
  ...
  ~JNICache() {
    ...
    env->DeleteGlobalRef(builderClass);
    env->DeleteGlobalRef(passManagerClass);
...
ScopedPassManager JNIContext::unwrapPassManager(JNIEnv *env,
                                                jobject passManager) {
  auto jPassManagerClass = env->GetObjectClass(passManager);
  auto cache = sharedCache.get();
  // If this assertion trips, consult the comment in
  // `Java_MLIR_Context_create` below.
  assert(cache);
  assert(cache->isValidForPassManagerClass(env, jPassManagerClass));
  auto opaquePointer = cache->getOpaquePointerFromJavaObject(env, passManager);
  return ScopedPassManager::getFromOpaquePointer(opaquePointer);
}
