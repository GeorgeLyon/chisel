...
public class PassManager extends NativeObject {
	...
	/**
	 * In MLIR Passes are mostly represented as `unique_ptr<Pass>`, and that value
	 * is more often than not passed directly to `PassManager::addPass`. Since some
	 * bound languages don't have support for something like `unique_ptr`, we simply
	 * define all of the `create...` methods directly on PassManager as `add...`
	 * methods.
	 */
